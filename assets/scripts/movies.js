...
async function setGenres(){
    let response = await fetch('https://api.themoviedb.org/3/genre/movie/list?language=fr', options)
...
function filter(pageId){
    ...
    let sort = getSort()
    let fetchAdress = 'https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=fr-FR'
...
async function getMovies(fetchAdress){
    if(fetchAdress === undefined){
        fetchAdress = 'https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=fr-FR&page=1&sort_by=popularity.desc'
    }
    $('.movies').empty()
    ...
    .then(movies => {
        movies.results.forEach(movie =>{
            ...
            if (movie.poster_path != null){
                $('.movies').append('<a href="details.html?movie_id='+movie.id+'"><div class="movie_poster"><div class="infos"><div class="infos_title"><h2>'+movie.title+'</h2></div><div class="infos_category"></div><div class="infos_synopsis"><p>'+synopsis+'</p></div></div><img src="https://image.tmdb.org/t/p/w500'+movie.poster_path+'"></div></a>')
            }
        })
    })
}
